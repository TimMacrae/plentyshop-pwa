<template>
  <header ref="referenceRef" :class="headerClass" class="relative w-full @md:sticky @md:shadow-md z-sticky">
    ...
    <nav v-if="viewport.isGreaterOrEquals('lg')" ref="floatingRef" @mouseleave="onMouseLeave">
      <ul class="flex flex-wrap px-6 py-0 bg-black border-b border-b-black border-b-solid" @focusout="onNavBlur">
        ...
        <li v-for="(menuNode, index) in categoryTree" v-else :key="index" @mouseenter="onCategoryMouseEnter(menuNode)">
          <!-- CUSTOM NAVIGATION: desktop category buttons disabled
               (custom nav is rendered in ui/Header.vue via navigationLinks) -->
          <!--
          <NuxtLink
          ...
          <div
            ...
            :style="style"
            class="hidden @md:grid gap-x-6 grid-cols-4 bg-black text-white shadow-lg p-6 pt-5 left-0 right-0 outline-none z-sticky max-h-[calc(100vh-300px)] overflow-y-auto"
                ...
                <ul>
                  <li>
                    <SfListItem
                      ...
                      class="mb-2 hover:!bg-black rounded font-medium typography-text-base"
                    >
    ...
    <template v-else>
      ...
      <SfDrawer
        ...
        v-model="isOpen"
        placement="left"
        class="right-12 max-w-96 bg-black overflow-y-auto z-drawer"
      >
        <nav>
